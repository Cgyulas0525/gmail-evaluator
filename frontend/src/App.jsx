...
const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:28088/api';
...
export default function App() {
  ...
  const handleSendCompose = async (event) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/emails/${selectedEmailFull.id}/${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteEmail = async (email) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/emails/${email.id}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleSyncAccount = async (account) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/accounts/${account.id}/sync`, { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleTestConnection = async (account) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/accounts/${account.id}/test`, { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDownloadAttachment = async (emailId, attachment) => {
    ...
    try {
      const res = await fetch(getAttachmentDownloadUrl(emailId, attachment.id));
      if (!res.ok) {
          ...
          <div>
            ...
            <div className="stats-grid">
              <div className="card stat-card">
                <span className="stat-title">Összes levelezés</span>
                <span className="stat-value">{stats ? stats.total_emails : 0}</span>
                <span className="stat-trend" style={{color: 'var(--primary)'}}>Aktív szinkronizáció</span>
              </div>
              <div className="card stat-card">
                <span className="stat-title">Sürgős levelek</span>
                <span className="stat-value" style={{color: 'var(--danger)'}}>
                  {stats ? (stats.priorities.urgent + stats.priorities.high) : 0}
                </span>
                <span className="stat-trend" style={{color: 'var(--text-secondary)'}}>Azonnali figyelmet igényel</span>
              </div>
              <div className="card stat-card">
                <span className="stat-title">Aktív Gmail fiókok</span>
                <span className="stat-value" style={{color: 'var(--success)'}}>
                  {accounts ? accounts.filter(a => a.status === 'active').length : 0}
                </span>
                <span className="stat-trend" style={{color: 'var(--text-secondary)'}}>Összesen: {accounts.length} fiók</span>
              </div>
            </div>
            ...
            <div className="dashboard-grid">
              ...
              <div className="card chart-container">
                <h3 style={{fontFamily: 'var(--font-title)', fontWeight: 600, fontSize: '1.1rem', marginBottom: '1rem'}}>
                  E-mail forgalom trend (Elmúlt 7 nap)
                </h3>
                <div className="bar-chart">
                  {stats && stats.trend ? stats.trend.map((day, idx) => {
                    ...
                    <div style={{color: 'var(--text-muted)', width: '100%', textAlign: 'center', paddingBottom: '3rem'}}>
            ...
            <div className="dashboard-grid">
              ...
              {/* Urgent Emails List */}
              <div className="card">
                <h3 style={{fontFamily: 'var(--font-title)', fontWeight: 600, fontSize: '1.1rem', marginBottom: '1.25rem', color: 'var(--danger)'}}>
        ...
        {activeTab === 'inbox' && (
          <div>
            <div className="page-header" style={{marginBottom: '1.5rem'}}>
              <div>
                <h1 className="page-title">Intelligens Inbox</h1>
                <p className="page-subtitle">Összegyűjtött levelek mélyreható AI kiértékelése</p>
              </div>
            </div>
        ...
        {activeTab === 'accounts' && (
          <div className="dashboard-grid" style={{gridTemplateColumns: '1.2fr 1fr'}}>
            ...
            <div>
              <div className="page-header" style={{marginBottom: '1.5rem'}}>
                <div>
                  <h1 className="page-title">Gmail Fiókok</h1>
                  <p className="page-subtitle">Add meg a követni kívánt e-mail címeket</p>
                </div>
              </div>

              {/* Add account card */}
              <div className="card" style={{marginBottom: '1.5rem'}}>
                <h3 style={{fontFamily: 'var(--font-title)', fontWeight: 600, fontSize: '1.1rem', marginBottom: '1.25rem'}}>
            ...
            <div className="card guide-container">
              <h3 style={{fontFamily: 'var(--font-title)', fontWeight: 700, fontSize: '1.25rem', color: 'var(--primary)'}}>
